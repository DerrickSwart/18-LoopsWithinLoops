...
def draw_L(window, circle, r, c):
    ...
    for k in range (3):
        for j in range(r+3):
            circle1 = rg.Circle(rg.Point(circle.center.x + (circle.radius *2*k), circle.center.y+circle.radius*2*j), circle.radius)
            circle1.fill_color = circle.fill_color
            circle1.attach_to(window)
            window.render(0.1)
    for f in range (3):
        for g in range (c):
            circle2= rg.Circle(rg.Point(circle1.center.x+circle1.radius *g*2, circle1.center.y - circle1.radius*2*f), circle1.radius)
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range (n):
        for j in range (n-k):
            ...
            up_left.y = rectangle.get_upper_left_corner().y + (rectangle.get_height()*(j+k))
